...
@Component({
  selector: 'app-login',
  standalone: true,
  imports: [HeaderComponent, FooterComponent, ReactiveFormsModule, HttpClientModule,MemberComponent],
...
export class LoginComponent {
  ...
  findAUser(email: string, password: string) {
    ...
    this.http.post<{ success: boolean, message: string, user?: Member }>(this.data.apiEndpoint + '/member/login', this.checkoutForm.value, { headers })
      .subscribe(response => {
